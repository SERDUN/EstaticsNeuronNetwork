...
from Data import train_data_0, test_data_0, train_data_3, test_data_1
...
#     plt.plot(x, y)
#
#     plt.legend(["not Calm condition"])
# plt.gca().invert_yaxis()
# plt.show()
#
#
size_list = len(test_data_1)
for i in range(size_list):
    x = test_data_1[i]["xReal"]
    y = test_data_1[i]["yReal"]
    plt.axis('off')
    plt.plot(x, y)
    plt.gca().invert_yaxis()
    plt.savefig("calm/train/calm/"+str(i)+".png")
    plt.show()
